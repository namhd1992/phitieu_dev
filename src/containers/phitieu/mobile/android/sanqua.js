...
class Lucky_Rotation extends React.Component {
	...
	componentDidMount(){
		...
		if(horizontal){
			...
			// 	x: bg_x*0.35,
			// 	y: bg_y*0.15,
			// 	text:"Nhanh tay giật giải IP12 trị giá 50 triệu",
			// 	fontSize: 14,
			// 	fontFamily: 'Calibri',
			// 	fill: 'yellow',
			// 	width: 380,
			...
			if(isPlay){
				if(countDart>0){
					...
					if(dartPositionY >y){
						...
						this.showTextWarning("Vuốt lên để phi tiêu!", 150);
	...
	autoPlay=()=>{
		...
		if(countDart>0){
			...
			this.props.gds(1,sessionId, user.Token, code_key, "android", 0, 0, 0, delta).then(()=>{
				var data=this.props.dataUserSpin;
				if(data.Status===0){
					if(!isChangetab){
	...
	render() {
		...
		const {isVideo, fullScreen, horizontal, auto_play, msg, listTop, duatop, isLoading,msg_err}=this.state;
